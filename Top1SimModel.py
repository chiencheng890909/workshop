...
class Top1SimModel(SimModel):
    ...
    def train_splitnn(self, data1, data2, labels, data_cache_path=None, scale=False, class_weight=True):
        ...
        if self.drop_key:
            if self.drop_A and self.drop_B:
                ...
                num_features = data1.shape[1] + data2.shape[1] - self.num_common_features
                input_dims = [self.data1_shape[1] - self.num_common_features, self.data2_shape[1]]
            elif self.drop_B:
                num_features = data1.shape[1] + data2.shape[1] - self.num_common_features
                input_dims = [self.data1_shape[1], self.data2_shape[1] - self.num_common_features]
        else:
            num_features = data1.shape[1] + data2.shape[1]
            input_dims = [self.data1_shape[1], self.data2_shape[1]]
        ...
        for epoch in range(self.num_epochs):
            ...
            if self.metrics[0] in ['auc', 'f1_score', 'average_precision', 'accuracy']: # 'r2_score'
